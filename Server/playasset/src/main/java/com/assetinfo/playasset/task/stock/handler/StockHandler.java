...
public class StockHandler {
    ...
    public Mono<ServerResponse> findAllTmpNews(ServerRequest request) {
        String keywords = request.attribute("keywords").toString();
        Flux<TmpNewsEntity> list = stockSvc.findAllTmpNews("삼성");
        return ServerResponse.ok().contentType(MediaType.TEXT_EVENT_STREAM).body(list, TmpNewsEntity.class);
    }
